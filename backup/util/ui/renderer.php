...
class core_backup_renderer extends plugin_renderer_base {
    ...
    public function get_status_display($statuscode, $backupid, $restoreid=null, $operation='backup') {
        if ($statuscode == backup::STATUS_AWAITING || $statuscode == backup::STATUS_EXECUTING) {  // In progress.
            $progresssetup = array(
                'backupid' => $backupid,
                'restoreid' => $restoreid,
                'operation' => $operation,
     ...
     *
     * @param int $userid The Moodle user id.
     * @param int $course The id of the course to get the backups for.
     * @return string $html The table HTML.
     */
    public function copy_progress_viewer(int $userid, int $courseid) : string {
        ...
        $copies = \core_backup\copy\core_backup_copy::get_copies($userid, $courseid);
