...
class course_copy_form extends \moodleform {
    ...
    public function definition() {
        ...
        $copies = \core_backup\copy\core_backup_copy::get_copies($USER->id, $course->id);
        if (!empty($copies)) {
        ...
        // Course category.
        $displaylist = \core_course_category::make_categories_list(array(
        ...
        $mform->addElement('advcheckbox', 'userdata',
            get_string('userdata', 'backup'),
        ...
        if (!empty($roles)) {
            ...
            foreach ($roles as $role) {
                $rolearray[$role->id] = $role->localname;
            }

            $select = $mform->addElement('select', 'manualenrols',
                get_string('manualenrols', 'backup'), $rolearray);
            $select->setMultiple(true);
            $mform->addHelpButton('manualenrols', 'manualenrols', 'backup');

            // By default set all available roles as selected.
            $select->setSelected(array_keys($rolearray));
        }

        $buttonarray=array();
        $buttonarray[] = $mform->createElement('submit', 'submitreturn', get_string('copyreturn', 'backup'));
        $buttonarray[] = $mform->createElement('submit', 'submitdisplay', get_string('copycourse'));
