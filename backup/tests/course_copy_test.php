...
class core_backup_course_copy_testcase extends advanced_testcase {
     ...
     * Test course copy
     */
    public function test_course_copy_test() {
        global $CFG;
        return;

        // Instantiate non interactive backup_controller
        $cc = new backup_controller(backup::TYPE_1COURSE, $this->course->id, backup::FORMAT_MOODLE,
            backup::INTERACTIVE_NO, backup::MODE_COPY, $this->userid, backup::RELEASESESSION_YES);

        // Enforce users to be included in backup.
        $cc->get_plan()->get_setting('users')->set_value(1);

        // Backup the course.
        $cc->set_status(backup::STATUS_AWAITING);
        $cc->execute_plan();
        $cc->destroy();
        ...
        $newcourseid = restore_dbops::create_new_course(
            $this->course->fullname, $this->course->shortname . '_2', $this->course->category);
        $rc = new restore_controller($backupdir, $newcourseid,
            backup::INTERACTIVE_NO, backup::MODE_ASYNC, $this->userid,
        ...
        $usersroles = get_users_roles($coursecontext, array_keys($users), false);

        error_log(print_r($usersroles, true));
        error_log(print_r($this->courseusers, true));
    ...
    public function test_course_copy() {
        ...
        $formdata->fullname = $this->course->fullname;
        $formdata->shortname = $this->course->shortname;
        $formdata->category = 1;
        $formdata->visible = 1;
        ...
        // Check restore was created successfully.
