...
/**
 * Test the describe image provider class for OpenAI provider methods.
 *
...
final class process_describe_image_test extends \advanced_testcase {
    ...
    protected function setUp(): void {
        ...
        $this->responsebodyjson = file_get_contents(self::get_fixture_path('aiprovider_openai', 'describe_request_success.json'));
        $this->manager = \core\di::get(\core_ai\manager::class);
        $this->provider = $this->create_provider(
            actionclass: \core_ai\aiactions\describe_image::class,
            actionconfig: [
                'systeminstruction' => get_string('action_describe_image_instruction', 'core_ai'),
            ],
        );
        $this->create_action();
    }
    ...
    public function test_create_request_object(): void {
        $processor = new \aiprovider_openai\process_describe_image($this->provider, $this->action);
    ...
    public function test_process_with_user_rate_limiter(): void {
        ...
        $this->create_action($user1->id);
        $processor = new \aiprovider_openai\process_describe_image($provider, $this->action);
        $result = $processor->process();
        ...
        $this->provider = $this->create_provider(\core_ai\aiactions\describe_image::class);
        $this->create_action($user1->id);
        $processor = new \aiprovider_openai\process_describe_image($provider, $this->action);
        $result = $processor->process();
        $this->assertTrue($result->get_success());
