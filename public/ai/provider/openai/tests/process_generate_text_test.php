...
final class process_generate_text_test extends \advanced_testcase {
    ...
    /**
     * Test incomplete Responses API payloads are returned as Moodle errors.
     */
    public function test_handle_api_success_with_incomplete_response(): void {
        $response = new Response(
            200,
            ['Content-Type' => 'application/json'],
            json_encode(['status' => 'incomplete', 'incomplete_details' => ['reason' => 'max_output_tokens']]),
        );
        $processor = new process_generate_text($this->provider, $this->action);
        $method = new \ReflectionMethod($processor, 'handle_api_success');
        ...
        $this->assertStringContainsString('max_output_tokens', $result['errormessage']);
    }
    ...
    /**
     * Test Responses configurations with custom extra parameters are rejected.
     */
    public function test_query_ai_api_rejects_custom_extra_parameters(): void {
        $this->provider = $this->create_provider(
            actionclass: \core_ai\aiactions\generate_text::class,
            actionconfig: ['modelextraparams' => '{"frequency_penalty": 1}'],
        );
        $processor = new process_generate_text($this->provider, $this->action);
        ...
        $this->assertStringContainsString('Custom extra parameters', $result['errormessage']);
    }
