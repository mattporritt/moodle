...
$string['ldap_encoding'] = 'Specify encoding used by LDAP server; most likely utf-8. Microsoft Active Directory (AD) v2 uses default platform encoding such as cp1252, cp1250, etc.';
...
$string['pluginname_desc'] = '<p>You can use a Lightweight Directory Access Protocol (LDAP) server to control enrolment. It is assumed your LDAP tree contains groups that map to the courses, and that each of those groups/courses have membership entries to map to students. Also, courses are defined as groups in LDAP, with each group having multiple membership fields (<em>member</em> or <em>memberUid</em>) that contain a unique identification of the user.</p><p>To use LDAP enrolment, users <strong>must</strong> have a valid  ID number field. The LDAP groups must have that ID number in the member fields for a user to be enrolled in the course. This works well with LDAP Authentication.</p><p>Enrolments are updated when the user logs in. You can also run a script <em>enrol/ldap/cli/sync.php</em> to keep enrolments in sync.</p><p>This plugin can also be set to automatically create new courses when new groups appear in LDAP.</p>';
