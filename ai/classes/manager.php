...
class manager {
     ...
     */
    private static function is_provider_action_enabled(string $plugin, string $actionclass, int $instanceid): bool {
        ...
        if ($instanceid === 0) {
            ...
            $providers = $manager->get_provider_instances(['id' => $instanceid]);
            $provider = reset($providers);
    ...
    public function is_action_enabled(string $plugin, string $actionclass, int $instanceid = 0): bool {
        if (str_contains($plugin, 'aiprovider')) {
            // Handle provider actions.
            return self::is_provider_action_enabled($plugin, $actionclass, $instanceid);
        } else {
            // Handle placement actions.
