...
class aiplacement_provider_table extends flexible_table implements dynamic_table {
    /** @var string The name of the plugin these actions related too */
    protected string $pluginname;

    /** @var array The list of providers this manager covers */
    protected array $providers;

    public function __construct(string $pluginname, array $providers) {
    ...
    protected function get_column_list(): array {
        return [
                'provider' => get_string('providers', 'core_ai'),
    ...
    /**
     * Show the provider column content.
     *
     * @param stdClass $row
     * @return string
     */
    protected function col_provider(stdClass $row): string {

        return $row->provider->get_name();
    }
    ...
    protected function col_enabled(stdClass $row): string {
        ...
        $enabled = true;
        if ($enabled) {
            $labelstr = get_string('disableplugin', 'core_admin', $row->provider->get_name());
        } else {
            $labelstr = get_string('enableplugin', 'core_admin', $row->provider->get_name());
        }
