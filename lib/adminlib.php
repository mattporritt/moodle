 ...
 * @param bool $unconditional if true overrides all values with defaults, true buy default
 * @param bool $outputsetting if true outputs defaults to CLI, false buy default
 */
function admin_apply_default_settings($node=null, $unconditional=true, $outputsetting=false, $admindefaultsettings=array()) {
    global $CFG;
    ...
    if ($node instanceof admin_category) {
        ...
        foreach ($entries as $entry) {
            $settingsoutput .= admin_apply_default_settings($node->children[$entry], $unconditional, $outputsetting, $admindefaultsettings);
        }
        ...
        foreach ($node->settings as $setting) {
            ...
            if (!array_key_exists($settingname, $admindefaultsettings)) {  // Only update a setting if not already processed.
                ...
                if ($outputsetting) { // Output default setting details if called from CLI.
                    $stringvlaues = array(
                            'name' => $settingname,
                            'defaultsetting' => var_export($defaultsetting, true) // Expand objects.
                    );
                    $settingsoutput .= get_string('cliupgradedefault', 'admin', $stringvlaues) . PHP_EOL;
                }
    ...
    if (($node instanceof admin_root) && (!empty($admindefaultsettings))) {
        $settingsoutput .= admin_apply_default_settings(null, $unconditional, $outputsetting, $admindefaultsettings);
    }
