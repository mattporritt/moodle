 ...
 * @since      3.7
 */
define(['jquery', 'core/ajax', 'core/str', 'core/fragment'], function($, ajax, Str, Fragment) {
    /**
    ...
    function updateBackupTableRow(backupid){
        ...
        ajax.call([{
            ...
            methodname: 'core_backup_async_backup_links',
            args: {
                'filename': filename,
                'contextid': contextid
            },
    ...
    function getAllBackupProgress() {
        var backupids = [];
        progressbars = $('.progress').find('.progress-bar').not('.complete').each(function(){
     ...
     * @public
     */
    Asyncbackup.asyncBackupStatus = function(backup, context, restore) {
        backupid = backup;
        contextid = context;
        restoreurl = restore;
