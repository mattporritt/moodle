...
class gradient {
    ...
    public static function generate_random_gradient(int $width, int $height, int $seed): string {
        ...
        $stops = [];
        $baseColor = utils::generate_preferred_color();

        for ($i = 0; $i < $numStops; $i++) {
            $color = ($i % 2 == 0) ? $baseColor : utils::complementary_color($baseColor); // alternate between the base color and its complementary color
